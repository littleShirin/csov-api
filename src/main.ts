...
async function main() {
	...
	const csovApi = await ApiPromise.create({
		provider: new WsProvider(config.SUBSTRATE.CSOV_WS_URL),
	...
	const app = new App({
		...
		controllers: getControllersForSpec(csovApi, api, specName.toString()),
		postMiddleware: [
			middleware.txError,
...
function startUpPrompt(wsUrl: string, chainName: string, implName: string) {
	...
	logger.info(
		`Connected to CSOV ${chainName} on the ${implName} client at ${config.SUBSTRATE.CSOV_WS_URL}`
	);
