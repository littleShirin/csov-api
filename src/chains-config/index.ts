...
export function getControllersForSpec(
	...
	// contoller config
	return getControllersFromConfig(csovApi,api, defaultControllers);
}
 ...
 */
function getControllersFromConfig(csovApi: ApiPromise,api: ApiPromise, config: ControllerConfig) {
	...
	return controllersToInclude.reduce((acc, [controllerName, shouldMount]) => {
		if (shouldMount) {
			acc.push(new controllers[controllerName]( csovApi, api));
